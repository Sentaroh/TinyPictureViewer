...
import com.sentaroh.android.TinyPictureViewer.CustomImageView.OnZoomChangedListener;
...
//			mGp.mCustomViewPager.setOffscreenPageLimit(3);
	...
	private void setPictureViewPagerListener() {
		mGp.customViewPager.addOnPageChangeListener(new OnPageChangeListener(){
			@Override
						...
						Thread th=new Thread(){
							...
							public void run() {
								...
								if (del_pic_item.getParentDirectory().startsWith(mGp.safMgr.getSdcardRootPath())) {
									...
									File lf=new File(del_pic_item.getParentDirectory()+"/"+del_pic_item.getFileName());
									rc_delete=lf.delete();
